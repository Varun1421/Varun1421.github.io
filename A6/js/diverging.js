...
d3.csv("data/airline_ticket_prices_dataset.csv").then(data => {
  ...
  const overallAverage = d3.mean(data, d => d.Price_USD);
  ...
  const avgPriceByAirline = Array.from(
    ...
    ([Airline, AvgPrice]) => ({
      Airline,
      AvgPrice,
      Difference: AvgPrice - overallAverage
    })
  );

  avgPriceByAirline.sort((a, b) => d3.descending(a.Difference, b.Difference));

  const minDiff = d3.min(avgPriceByAirline, d => d.Difference);
  const maxDiff = d3.max(avgPriceByAirline, d => d.Difference);
  ...
  chartGroup
    ...
    .attr("fill", d => color(d.Difference));
  ...
  chartGroup
    ...
    .text("Airline Ticket Price Difference from Overall Average");
  ...
  chartGroup
    ...
    .attr("y", height + 55)
    .attr("text-anchor", "middle")
    .text("Difference from Overall Average Ticket Price (USD)");
